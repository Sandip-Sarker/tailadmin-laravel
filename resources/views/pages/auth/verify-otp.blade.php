                                    ...
                                    <div class="space-y-5">
                                        ...
                                        <div>
                                            <label class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                                Enter OTP<span class="text-error-500">*</span>
                                            </label>
                                            <p class="text-xs text-gray-500 dark:text-gray-500 mb-4">We sent a 6-digit code to your email</p>
                                        </div>
                                        
                                        <!-- OTP Input Fields -->
                                        <div class="flex gap-2 justify-center" id="otpContainer">
                                            <input type="text" maxlength="1" class="otp-input w-12 h-12 text-center text-lg font-semibold border border-gray-300 rounded-lg dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 focus:ring-3 focus:outline-hidden" />
                                            <input type="text" maxlength="1" class="otp-input w-12 h-12 text-center text-lg font-semibold border border-gray-300 rounded-lg dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 focus:ring-3 focus:outline-hidden" />
                                            <input type="text" maxlength="1" class="otp-input w-12 h-12 text-center text-lg font-semibold border border-gray-300 rounded-lg dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 focus:ring-3 focus:outline-hidden" />
                                            <input type="text" maxlength="1" class="otp-input w-12 h-12 text-center text-lg font-semibold border border-gray-300 rounded-lg dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 focus:ring-3 focus:outline-hidden" />
                                            <input type="text" maxlength="1" class="otp-input w-12 h-12 text-center text-lg font-semibold border border-gray-300 rounded-lg dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 focus:ring-3 focus:outline-hidden" />
...
<script>
    $(document).ready(function() {
        ...
        $otpInputs.on('paste', function(e) {
            e.preventDefault();
            const index = $otpInputs.index(this);
            const pastedData = (e.originalEvent.clipboardData.getData('text')).replace(/[^0-9]/g, '').slice(0, $otpInputs.length - index);
            
            pastedData.split('').forEach((char, i) => {
                if (index + i < $otpInputs.length) {
                    $otpInputs.eq(index + i).val(char);
                }
            });
